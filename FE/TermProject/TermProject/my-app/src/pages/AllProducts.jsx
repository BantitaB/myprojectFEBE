...
const AllProducts = () => {
    ...
    const addToCart = async (product, quantityToAdd) => {
        try {
            ...
            setShowModal(true);  // แสดง Modal เมื่อเพิ่มสินค้าแล้ว
        } catch (error) {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/v1/products/allproducts');
        ...
        <div>
            ...
            <div style={{ paddingLeft: '110px', paddingTop: '130px' }}>
                <Breadcrumb>
                    ...
                    <Breadcrumb.Item active>สินค้าทั้งหมด</Breadcrumb.Item>
                </Breadcrumb>
            </div>
            <h1 className="text-center text-title" style={{ marginTop: '40px' }}>สินค้าทั้งหมด</h1>
