...
import { Card, Container, Row, Col, Button, CardText, Modal } from 'react-bootstrap';
...
const RecommendProducts = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:8080/api/v1/products/recommend');
    ...
    return (
        <div>
            <h1 className="text-center text-title" style={{ marginTop: '40px' }}>สินค้าแนะนำ</h1>
                            ...
                            return (
                                <Col md={4} key={product.product_id} className="d-flex justify-content-center">
                                    ...
                                    >
                                        ...
                                        <div className="recommend-badge">Recommend</div>
                                        <Card.Body>
                                            <Card.Title><strong>{product.name}</strong></Card.Title>
                                            <CardText style={{ color: "#5e5e5e" }}>ร้านค้า : {product.seller.name}</CardText>
                                            <CardText style={{ color: "#5e5e5e" }}>หมวดหมู่ : {product.categories.name}</CardText>
                                            <CardText style={{ color: "#5e5e5e" }}>สินค้าคงเหลือ : {product.inventory.quantity}</CardText>
